...
export default function PreferenceSetup({ onSave, onBack, initialPreferences }: PreferenceSetupProps) {
                  ...
                  {notes.map((note) => {
                    ...
                    return (
                      <button
                        ...
                        `}
                      >
                        {status === "preferred" && <Check className="w-3 h-3 inline mr-1" />}
                        {note}
                      </button>
                    )
                  })}
            ...
            {situationOptions.map((situation) => {
              ...
              return (
                <button
                  ...
                  `}
                >
                  <p className={`font-medium mb-1 ${isSelected ? "text-background" : "text-foreground"}`}>
                    {situation.label}
                  </p>
        ...
        <section className="border-t border-border pt-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="text-sm text-muted-foreground">
              <span className="font-medium text-foreground">{preferredNotes.length}</span> preferred notes, {" "}
              <span className="font-medium text-foreground">{dislikedNotes.length}</span> avoided, {" "}
              <span className="font-medium text-foreground">{situations.length}</span> situations
            </div>
            <Button 
              onClick={handleSave}
